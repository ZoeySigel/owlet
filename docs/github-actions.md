...
唯一必需的 Repository Secret：OWLET_DEPLOY_KEY，内容是本机 .tools/ssh/owlet_actions_ed25519 文件全文（包含 BEGIN/END 行）。在 Settings → Secrets and variables → Actions → New repository secret 中设置，禁止提交到代码或通过聊天发送。

另外需设置两个 Repository Variables：OWLET_DEPLOY_HOST 为服务器 IP，OWLET_KNOWN_HOST 为已核实的 known_hosts 记录（主机名、公钥类型、公钥三部分）。真实值不存入仓库。更换服务器必须通过可信渠道重新核对公钥，不自动信任 ssh-keyscan 结果。
